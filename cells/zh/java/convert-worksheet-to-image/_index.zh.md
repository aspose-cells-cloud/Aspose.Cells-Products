---
title: 将本地驱动器上的电子表格工作表转换为图像格式。
description: 此方法从本地文件系统读取电子表格文件，将其工作表转换为所需的图像格式（例如 svg、png 等），并返回转换结果。 \n必须正确指定源文件路径和目标格式。 \n确保拥有读取源文件和（如适用）写入转换后文件的必要权限。 \n转换过程完全在云服务器上进行，无需任何云存储或外部下载。 \n如果源文件不存在、无法访问，或在转换过程中出现错误，将抛出相应的异常。 \n支持的转换格式取决于可用的库及其功能。
kwords: Aspose Cells
url: /zh/net/convert-worksheet-to-image/
---
...
{{< blocks/products/cells/cells-cloud-languageindex indextitle="将 Excel 文件转换为其他格式" indexdesc="Aspose.Cells Cloud 提供对 Excel 文件格式转换的强大支持，这是一项复杂的过程。Aspose.Cells Cloud 支持 30 多种文件格式，包括 Excel、Pdf、Markdown、Json、XML、Csv、Html 等。" >}}
