---
title: 将本地驱动器上的电子表格表格转换为 CSV 文件。
description: 此方法从本地文件系统读取电子表格文件，将其表格转换为所需的 CSV 文件，并返回转换结果。 \n必须正确指定源文件路径和目标格式。 \n确保具备读取源文件和（如适用）写入转换后文件的必要权限。 \n转换过程完全在云服务器上执行，无需任何云存储或外部下载。 \n如果源文件不存在、无法访问，或在转换过程中发生错误，将抛出相应的异常。 \n支持的转换格式取决于可用库及其功能。
kwords: Aspose Cells
url: /zh/java/convert-table-to-csv/
---
